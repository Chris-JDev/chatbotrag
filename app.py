...
# ---------- Rule-Based Campaign Classifier from RAG & AIDA PDF ----------
CAMPAIGN_PATTERNS = {
...
def get_prompt():
    return ChatPromptTemplate.from_messages([
        SystemMessagePromptTemplate.from_template(
            "You are a marketing QA system. Use only provided docs and structure answers using the AIDA model."
        ),
...
def main():
    ...
    with st.sidebar:
        ...
        if cat != st.session_state.selected_category: ss.selected_category = cat
        ...
        if st.button("Generate Quick Ideas"):
            with st.spinner("Generating…"):
                if st.session_state.vector_store:
                    ...
                    prompt = (f"Based on these docs:\n{ctx}\nGenerate 5 quick marketing ideas for {st.session_state.selected_category}. "
        ...
        with c2:
            if st.button("Clear Chat"): st.session_state.messages=[] and st.success("Cleared!")
        if st.session_state.available_histories:
            h = st.selectbox("Load chat", [""]+st.session_state.available_histories)
            if h and st.button("Load Selected Chat"): msgs=load_history(h); msgs and (st.session_state.messages:=msgs) and st.success("Loaded!")
    ...
    if ui := st.chat_input("Ask your marketing question…"):
        ...
        if st.session_state.vector_store:
            qa=init_qa_chain(); ans=qa.run(ui) if qa else "Error: QA unavailable"
        else:
