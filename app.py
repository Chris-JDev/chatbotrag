...
# Global CSS for larger font
st.markdown(
    """
    <style>
    .assistant-response {font-size: 20px; color: #333; margin-bottom: 1em;}
    .user-message    {font-size: 18px; font-weight: bold; margin-bottom: 0.5em;}
    </style>
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button('Generate Quick Ideas', key='quick_ideas'):
            if not st.session_state.vector_store:
                ...
                with st.spinner('Generating ideas…'):
                    retr = get_retriever()
                    docs = retr.get_relevant_documents(f'Generate 5 marketing ideas for {st.session_state.selected_category}')
                ...
                st.markdown(f"<div class='assistant-response'>{ideas}</div>", unsafe_allow_html=True)
        st.markdown('---')
        st.markdown('### AIDA Model Explanation')
        if st.button('Show AIDA Explanation', key='aida_explain'):
            st.markdown(f"<div class='assistant-response'>{AIDA_DESCRIPTION}</div>", unsafe_allow_html=True)
        st.markdown('---')
        ...
        if st.button('Generate AIDA Plan', key='aida_plan'):
            if not st.session_state.vector_store:
                ...
                with st.spinner('Generating AIDA plan…'):
                    retr = get_retriever()
                    docs = retr.get_relevant_documents(f'Generate an AIDA-structured marketing plan for {st.session_state.selected_category}')
                    ctx = '\n\n'.join(d.page_content for d in docs)
                    prompt = (f'Context:\n{ctx}\n\nQuestion: Generate a full marketing plan for {st.session_state.selected_category}, structured according to the AIDA model (Attention, Interest, Desire, Action).')
                    r = st.session_state.llm.invoke(prompt)
                    plan = getattr(r,'content',str(r))
                st.markdown(f"<div class='assistant-response'>{plan}</div>", unsafe_allow_html=True)
        st.markdown('---')
        st.header('Chat Management')
        c1,c2 = st.columns(2)
        with c1:
            if st.button('Save Chat', key='save_chat'):
                if save_history(st.session_state.messages): st.success('Chat saved!')
        with c2:
        ...
        if st.session_state.available_histories:
            ...
            if h and st.button('Load Selected Chat', key='load_chat'):
                msgs = load_history(h)
                if msgs: st.session_state.messages=msgs; st.success('Chat loaded!')
    ...
    for msg in st.session_state.messages:
        cls = 'assistant-response' if msg['role']=='assistant' else 'user-message'
    ...
    if st.session_state.chat_started:
        if ui := st.chat_input('Ask your marketing question…', key='chat_input'):
            ...
            qa = init_qa_chain()
            if qa: ans = qa.run(ui)
            else: ans = getattr(st.session_state.llm.invoke(ui),'content',str(_))
            st.session_state.messages.append({'role':'assistant','content':ans})
            st.markdown(f"<div class='assistant-response'>{ans}</div>", unsafe_allow_html=True)
